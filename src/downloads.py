...
def download_johd_paper(url, fn, method='selenium', force_download=False):
    ...
    assert method in ['selenium', 'api', 'meta/pdf']
    ...
    if method == 'selenium':
        ...
        get_whole_js = """return function() {
            let articleElement = document.querySelector('article[data-testid="article-content"]');
            return articleElement.textContent;
        }()"""
        get_refs_js = """return function() {
            //let refsElement = document.querySelector('div[class="article-references"]');
            let refsElement = document.getElementsByClassName('article-references')[0];
            return refsElement.textContent;
        }()"""
        for i in range(100):
        ...
        whole_content = browser.execute_script(get_whole_js)
        whole_content += browser.execute_script(get_refs_js)
        ...
        num = url.rsplit('.',1)[1]
        url = f'https://taskmaster.ubiquity.press/api/article/up-j-johd/{num}/html?format=json'
        ...
        with open(fn + '.pdf', 'wb') as pdf_file:
...
def download_paper(doi, url, dest_dir, force_download=False):
    ...
    if url.startswith('https://openhumanitiesdata.metajnl.com/'):
        download_johd_paper(url, fn, method='meta/pdf', force_download=force_download)
    elif '://' in doi:
...
def redownload_johd():
    df = pd.read_csv(DATASET_FN, index_col=0)
    for index, row in df[df['source'] == 'original-johd'].iterrows():
        doi = row['data_paper_doi']
        print(f'\n{index+1}/{len(df)} {doi} ', end='')
        download_paper(doi, row['data_paper_url'], DATA_PAPERS_DIR, force_download=False)
